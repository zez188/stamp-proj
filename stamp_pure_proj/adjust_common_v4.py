...
def Perspective_transform(box, original_img, rect):
    ...
    # if abs(ori_h - cutted_H) < 20 and abs(ori_w - cutted_W) < 20:
    #     return original_img

    # 原图中的四个顶点,与变换矩阵
    # pts2 = np.float32([[int(cutted_W+1), int(cutted_H+1)], [0, int(cutted_H+1)], [0, 0], [int(cutted_W+1), 0]])
    pts2 = np.float32([[0, 0], [int(cutted_W + 1), 0], [int(cutted_W + 1), int(cutted_H + 1)], [0, int(cutted_H + 1)]])

    # print("pts1:", pts1)
    # print("pts2:", pts2)
    # print(cutted_H, cutted_W, "\n")
...
if __name__ == "__main__":
    ...
    common_save_path = adjust_single_img(file_path)
