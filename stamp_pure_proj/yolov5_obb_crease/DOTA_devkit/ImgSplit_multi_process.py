...
class splitbase():
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        ...
        if (self.padding):
            ...
            cv2.imencode('.jpg',outimg)[1].tofile(outdir)
        else:
            # cv2.imwrite(outdir, subimg)
            cv2.imencode('.jpg',subimg)[1].tofile(outdir)
...
if __name__ == '__main__':
    split = splitbase(basepath=r'/data/ZGM/yolov5_obb/dataset/test',
                      outpath=r'/data/ZGM/yolov5_obb/dataset/test_split',
                      gap=200,
                      subsize=1024,
                      num_process=16)
    split.splitdata(1)