...
from utils_obb.general import (LOGGER, Timeout, check_requirements, clip_coords, increment_path, is_ascii, is_chinese,
                           try_except, user_config_dir, xywh2xyxy, xyxy2xywh)
from utils_obb.metrics import fitness
from utils_obb.rboxs_utils import poly2hbb, poly2rbox, rbox2poly
...
def check_font(font='Arial.ttf', size=10):
    ...
    print("font.exists()", font.exists())
    print("str(font)", str(font))
    try:
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        print("self.pil, is_ascii(example), is_chinese(example)", self.pil, is_ascii(example), is_chinese(example))
        if self.pil:  # use PIL
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                print("box_label() label", label)
                w, h = self.font.getsize(label)  # text width, height
                x_outside = box[0] >= 0  # label_x outside box
                y_outside = box[1] - h >= 0  # label_y outside box
                # 画label文字的框
                self.draw.rectangle([box[0] if x_outside else 0,
                                     box[1] - h if y_outside else box[1],
                                     box[0] + w + 1 if x_outside else w + 1,
                                     box[1] + 1 if y_outside else box[1] + h + 1], fill=color)
                # self.draw.text((box[0], box[1]), label, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
                # self.draw.text((box[0], box[1] - h if y_outside else box[1]), label, fill=txt_color, font=self.font)
                self.draw.text((box[0] if x_outside else 0, box[1] - h if y_outside else box[1]), label, fill=txt_color, font=self.font)
        else:  # cv2
