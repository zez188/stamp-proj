...
def detectFront(file_ori_path, file_com_path, file_pos_path, file_per_path, file_perf_dir, file_cls_name, mat_com, use_ZH):
    ...
    abra_real_xyxy_list = label_img_v8.handle_xyxy_cvt_pos(abra_xyxy_list, mat_com)
    # 缺损的在原图上的标框位置信息转换为在矫正图上的标框信息
    defect_real_xyxy_list = label_img_v8.handle_xyxy_cvt_pos(defect_xyxy_list, mat_com)
    # 折痕的去掉一些在边界有误检嫌疑的框
    crease_real_poly_list = label_img_v8.handle_crease_remove_margin(crease_poly_list, img_com)
    ...
    ele_dmg_score_inputs, ele_dmg_xyxy_list = label_img_v8.handle_ele_overlap(cls_labels_list, img_com)
    cls_labels_list.append(ele_dmg_xyxy_list)

    # handle_perf_transform() 计算：齿孔四条边的标框位置转信息转换为矫正图上的标框位置信息
    perf_real_xyxy_list = label_img_v8.handle_perf_trans_with_extend(perf_xyxy_list, img_com, img_perf)
    ...
    labelled_img = label_img_v8.label_all_cls(cls_labels_list, anno, isChinese=True)
...
def detectBack(file_com_path, file_cls_name, use_ZH):
    ...
    # 污渍、黄斑执行 标框
    anchored_back_img = label_img_v8.handle_back_mould_stain(stain_xyxy_list, anno_back, use_ZH)
