...
"""
detect_all_v8.py 修复了缺损训练时用原图、实际测试用矫正图的错误
"""
# 正面
# index          0          1         2        3           4           5             6            7          8
cls_front = ["abrasion", "defect", "stain", "crease", "ele_damage", "perf_damage", "reflect", "pat_pos", "fade"]
#                 0       1      2       3        4        5           6           7         8
cls_front_ZH = ["磨损", "缺损", "污渍", "折痕", "票面要素", "齿孔", "反射效果变暗", "图案位置", "褪色"]
# 背面
# index          0              1          2
cls_back = ['back_yellow', 'back_stain', 'torn']
#                  0          1         2
cls_back_ZH = ["背面泛黄", "背面污渍", '撕裂']
...
# 正面检测、标框、打分
def detectFront(file_ori_path, file_com_path, file_pos_path, file_per_path, file_perf_dir, file_cls_name, mat_com,
                use_ZH):
    """
    ...
    print("file_ori_path", file_ori_path, flush=True)
    ...
    # 缺损执行 获取标记信息
    defect_xyxy_list = detect_defect.main(file_ori_path)
    # 污渍、黄斑执行 获取标记信息
    ...
    # 标框部分
    print("标框部分")
    h, w = img_com.shape[0:2]
    font_size_zh = int(pow(h * w, 0.5)) // 100
    print("font_size_zh", font_size_zh)
    # font_size_en = font_size_zh//8
    font_size_en = 5

    if use_ZH:
        anno = Annotator(img_com_copy, line_width=5, font_size=font_size_zh, example="汉字")
    else:
        anno = Annotator(img_com_copy, line_width=5, font_size=font_size_en)

    # 磨损的在原图上的标框位置信息转换为在矫正图上的标框信息
    abra_real_xyxy_list = label_img_v9.handle_xyxy_cvt_pos(abra_xyxy_list, mat_com)
    # 缺损的在原图上的标框位置信息转换为在矫正图上的标框信息
    defect_real_xyxy_list = label_img_v9.handle_xyxy_cvt_pos(defect_xyxy_list, mat_com)
    # 折痕的去掉一些在边界有误检嫌疑的框
    crease_real_poly_list = label_img_v9.handle_crease_remove_margin(crease_poly_list, img_com)

    front_cls_labels_list = [ele_xyxy_list,
                             abra_real_xyxy_list,
                             defect_real_xyxy_list,
                             stain_xyxy_list,
                             crease_real_poly_list]
    # print("front_cls_labels_list before", front_cls_labels_list)

    # handle_ele_overlap() 计算：ele与其他5小类的重叠信息（ele_damage的标框位置信息）+ ele_damage打分所需inputs
    ele_dmg_score_inputs, ele_dmg_xyxy_list = label_img_v9.handle_ele_overlap(front_cls_labels_list, img_com)
    front_cls_labels_list.append(ele_dmg_xyxy_list)

    # handle_perf_transform() 计算：齿孔四条边的标框位置转信息转换为矫正图上的标框位置信息
    perf_real_xyxy_list = label_img_v9.handle_perf_trans_with_extend(perf_xyxy_list, img_com, img_perf)
    front_cls_labels_list.append(perf_real_xyxy_list)
    front_cls_labels_list.append(refl_xyxy_list)
    front_cls_labels_list[3] = mould_stain_xyxy_list

    # label_all_cls() 执行所有框在矫正图上的标注
    labelled_img = label_img_v9.label_all_cls(front_cls_labels_list, anno, is_chinese=True, is_front=True)
        ...
        for i in range(len(cls_front_ZH)):
            if i == 7:
                if score_front_nums[i] > 0:
                    ...
                    score_front_list[i] = cls_front_ZH[i] + "左偏" + ":" + str(abs(score_front_nums[i]))
                else:  # pos_lr_score = 0
        ...
        for i in range(len(cls_front)):
            if i == 7:
                if score_front_nums[i] > 0:
                    ...
                    score_front_list[i] = cls_front[i] + "左偏" + ":" + str(abs(score_front_nums[i]))
                else:  # pos_lr_score = 0
...
def detectBack(file_com_path, file_cls_name, use_ZH):
    ...
    # line_thickness = 5
    if use_ZH:
        anno_back = Annotator(img_com_copy, line_width=5, font_size=font_size_zh, example="汉字")
    else:
        anno_back = Annotator(img_com_copy, line_width=5, font_size=font_size_en)
    ...
    back_cls_labels_list = [stain_xyxy_list,
                            torn_poly_list]
    anchored_back_img = label_img_v9.label_all_cls(back_cls_labels_list, anno_back, is_chinese=True, is_front=False)
    # # 污渍、黄斑执行 标框
    # anchored_back_img = label_img_v9.handle_back_mould_stain(stain_xyxy_list, anno_back, use_ZH)
    ...
    if use_ZH:
        ...
        # 撕裂分数输入
        score_back_list[2] = cls_back_ZH[2] + ':' + str(torn_score)
    else:
        # 背泛黄分数输入
        ...
        # 撕裂分数输入
        score_back_list[2] = cls_back[2] + ':' + str(torn_score)
