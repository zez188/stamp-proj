...
def Perspective_transform(box, original_img, rect):
    ...
    if abs(ori_h - cutted_H) < 20 or abs(ori_w - cutted_W) < 20:
        if abs(ori_h - cutted_H) < 20:
            padding_value_arr = original_img[:, 0]
        else:
            padding_value_arr = original_img[0, :]
        # print(padding_value_arr, padding_value_arr.shape)
        values_1, values_2, values_3 = padding_value_arr[:, 0], padding_value_arr[:, 1], padding_value_arr[:, 2]

        pts2 = np.float32([[0, 0],
                           [int(cutted_W), 0],
                           [int(cutted_W), int(cutted_H)],
                           [0, int(cutted_H)]])
        M = cv2.getPerspectiveTransform(pts1, pts2)
        res = cv2.warpPerspective(original_img, M, (int(cutted_W), int(cutted_H)))

        padding_value_aver = (np.average(values_1),
                              np.average(values_2),
                              np.average(values_3))
        print("padding_value_aver", padding_value_aver)
        res = cv2.copyMakeBorder(res, extend_len, extend_len, extend_len, extend_len,
                                 cv2.BORDER_CONSTANT, value=padding_value_aver)
        return res, M

    # 原图中的四个顶点,与变换矩阵
    # pts2 = np.float32([[int(cutted_W+1), int(cutted_H+1)], [0, int(cutted_H+1)], [0, 0], [int(cutted_W+1), 0]])
...
if __name__ == "__main__":
    file_path = r"1994-8（4-1）1A.jpg"
    pref_save_path, _1, _2 = adjust_perf_img(file_path)
    print("pref_save_path", pref_save_path)
    # file_dir = r"E:\stamp_proj\stamp_dataset\stamp_elem_set\total"
    # file_list = os.listdir(file_dir)
    # # save_dir = ""
    # for file in file_list:
    #     file_path = os.path.join(file_dir, file)
    #     pref_save_path, _ = adjust_pref_img(file_path)
    #     print("pref_save_path", pref_save_path)
