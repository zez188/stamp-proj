...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        save_box_stain_txt_path = os.path.join(save_box_single_dir, detect_pic_name[:-4] + "__stain" + '.txt')
        save_box_mould_txt_path = os.path.join(save_box_single_dir, detect_pic_name[:-4] + "__mould" + '.txt')
        with open(save_box_stain_txt_path, "w") as fw:
            pass
        with open(save_box_mould_txt_path, "w") as fw:
            pass
        order_num = 0
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # 统计标签的数量
                        if names[c] == 'mould':
                            save_txt_path = os.path.join(save_box_single_dir, detect_pic_name[:-4] + "__mould" + '.txt')
                            if mould_num == 0:
                            ...
                            print("detect_stain_ori.py name[" + str(c) + "] =", names[c])
                            print("detect_stain_ori.py pic_num, mould_num = ", pic_num, mould_num)
                            mould_num += 1

                        elif names[c] == 'stain':
                            save_txt_path = os.path.join(save_box_single_dir, detect_pic_name[:-4] + "__stain" + '.txt')
                            if stain_num == 0:
                                with open(save_txt_path, "w", encoding="UTF-8") as fw:
                                    fw.write(write_content)
                            else:
                            ...
                            print("detect_stain_ori.py name[" + str(c) + "] =", names[c])
                            print("detect_stain_ori.py pic_num, stain_num = ", pic_num, stain_num)
                            stain_num += 1
...
def parse_opt(file_dir, side="front"):
    ...
    if side == "front":
        ...
        parser.add_argument('--weights', nargs='+', type=str, default='weights/stain_back_best.pt', help='model path(s)')
...
def main(file_dir, side="front"):
    print("detect_stain.py os.getcwd()", os.getcwd())
    opt = parse_opt(file_dir, side)
...
if __name__ == "__main__":
    # opt = parse_opt()
    file_dir = r"../detect_imgs_small/1994-8（4-1）1A.jpg"
